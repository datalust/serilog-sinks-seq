...
namespace Serilog.Sinks.Seq
{
    ...
    class ConstrainedBufferedFormatter : ITextFormatter
    {
        ...
        {
            // If the limit is so constrained as to disallow sending 1024 bytes + packaging, that's okay - we'll just drop
            // the placeholder, too.
            var sample = jsonLine.Substring(0, (int)Math.Min(jsonLine.Length, eventBodyLimitBytes));
            return new LogEvent(
                logEvent.Timestamp,
                LogEventLevel.Error,
                exception: null,
